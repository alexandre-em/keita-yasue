...
export const sendMessageMail = async (
  to: ContactType,
  from: ContactType,
  message: string,
) => {
  ...
  const emailParams = new EmailParams()
    ...
    .setTemplateId(
      process.env["NEXT_PUBLIC_MAILSENDER_MESSAGE_TEMPLATE_ID"] ?? "",
    )
...
export const sendReservationMail = async (
  ...
  const emailParams = new EmailParams()
    ...
    .setSubject(`Your lesson with Keita the ${data.date}`)
    .setTemplateId(
      process.env["NEXT_PUBLIC_MAILSENDER_RESERVATION_TEMPLATE_ID"] ?? "",
    )
    .setPersonalization(personalization);
...
export const sendValidationReservationMail = async (
  to: ContactType,
  data: ValidationReservationData,
) => {
  const mailersend = new MailerSend({
    apiKey: process.env["NEXT_PUBLIC_MAILSENDER_API_TOKEN"] ?? "",
  });
  ...
  const dateLocale = startDate.toLocaleString("en-US", {
    day: "numeric",
    month: "long",
    year: "numeric",
  });
  ...
  const emailParams = new EmailParams()
    ...
    .setAttachments([
      new Attachment(
        attachmentContent,
        `invite-${to.name.trim()}-${formatToICSDate(startDate)}.ics`,
      ),
    ])
    .setTemplateId(
      process.env[
        "NEXT_PUBLIC_MAILSENDER_VALIDATION_RESERVATION_TEMPLATE_ID"
      ] ?? "",
    )
    .setPersonalization(personalization);
