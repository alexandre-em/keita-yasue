...
import {
  statusCancellationLevel,
  statusColor,
  statusValidationLevel,
} from "@/constants";
...
export default async function ReservationId({ params }: IdParamsType) {
  ...
  const statusArray =
    reservation?.status === "CANCELLED" || reservation?.status === "TO_CANCEL"
      ? statusCancellationLevel
      : statusValidationLevel;
  const statusLevelIndex = reservation?.status
    ? statusArray.indexOf(reservation.status)
    : 0;
    ...
    <div className="m-5 mt-0 w-full">
      ...
      <div className="flex flex-wrap items-center justify-between">
        ...
        {(user.role !== "ADMIN" || user.id !== reservation.author.id) && (
          <ChatDestButton
            user={user}
            dest={user.role === "ADMIN" ? reservation.author : admin}
          />
        )}
      </div>
        ...
        <Card className="my-5 grow">
          <CardHeader>
            ...
            {reservation.updatedAt && (
              <p className="text-xs italic">
                Updated the {reservation.updatedAt.toLocaleString()}
              </p>
            )}
          </CardHeader>
          <CardContent>
          ...
          <CardFooter>
            <div className="flex flex-wrap">
              ...
              {user.role === "USER" &&
                (reservation.status === "TO_VALIDATE" ||
                ...
                <>
                  ...
                  {(reservation.status === "DONE" ||
                    ...
                    <UpdateReservationReview
                      id={id}
                      review={reservation.studentReview}
                    />
                  )}
                </>
              )}
